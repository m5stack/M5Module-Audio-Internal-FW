...
{
    HAL_GPIO_WritePin(LIN_MIC_PU_EN_GPIO_Port, LIN_MIC_PU_EN_Pin, GPIO_PIN_RESET);
...
void reg_init(void)
{
    ...
    // Detect whether headphones are inserted
...
void Slave_Complete_Callback(uint8_t *rx_data, uint16_t len)
{
    ...
    if (len > 1) {
        if (rx_data[0] == 0x00 && len == 2) {
            ...
            if (rx_data[1] == NATIONAL_STANDARD || rx_data[1] == AMERICA_STANDARD) {
            ...
            if (rx_data[1] == 1) {
                if (lin_mic_pc_en_reg != get_lin_mic_pc_en_status() || hp_mode_reg != get_hp_mode_status() ||
                    rgb_light_reg != get_rgb_light_status()) {
                    write_flash(lin_mic_pc_en_reg, hp_mode_reg, rgb_light_reg);
                }
            }
